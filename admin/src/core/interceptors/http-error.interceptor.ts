    ...
    return next.handle( request ).pipe(
      ...
      catchError((err) => {
        ...
        if (statusCode == 400) { // Bad Request
          ...
          console.warn(`${request.urlWithParams} failed with error: ` + JSON.stringify(err));
          // Do not show modal for 404 to avoid disruptive UX on non-critical missing resources
