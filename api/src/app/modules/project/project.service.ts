...
export class ProjectFindCriteria {
  ...
  applyFindCriteria(query: SelectQueryBuilder<Project>) {
    ...
    if (this.districtId) {
      query.andWhere('p.district_id = :districtId', {
        districtId: `${this.districtId}`,
      });
    }
    if (
      this.includeWorkflowStateCodes &&
      this.includeWorkflowStateCodes.length > 0
    ) {
      query.andWhere('p.workflow_state_code IN (:...workflowStateCodes)', {
        workflowStateCodes: this.includeWorkflowStateCodes,
      });
    }
    if (this.likeForestClientName) {
      // Case insensitive search
      query.andWhere('upper(forestClient.name) like :forestClientName', {
        forestClientName: `%${this.likeForestClientName.toUpperCase()}%`,
      });
    }
    if (this.commentingOpenedOnOrAfter) {
      query.andWhere('p.commenting_open_date >= :openDate', {
        openDate: `${this.commentingOpenedOnOrAfter}`,
      });
    }
    if (
      this.includeForestClientNumbers &&
      this.includeForestClientNumbers.length > 0
    ) {
      query.andWhere('p.forest_client_number IN (:...forestClientNumbers)', {
        forestClientNumbers: this.includeForestClientNumbers,
      });
    }
  }
...
> {
  ...
  async isCreateAuthorized(
    ...
    return (
      user.isForestClient &&
      dto.forestClientNumber != null &&
      user.isAuthorizedForClientId(dto.forestClientNumber)
    );
  }
  ...
  async isUpdateAuthorized(
    ...
    if (
      !user.isForestClient ||
      !user.isAuthorizedForClientId(entity.forestClientId)
    ) {
      return false;
    }
    ...
    if (
      ...
      this.logger.debug(
        `Not allowed to edit FOM in state other than INITIAL, COMMENT_OPEN and COMMENT_CLOSED.`
      );
      return false;
    }
    ...
    if (WorkflowStateEnum.INITIAL !== entity.workflowStateCode) {
      if (entity.commentingOpenDate !== dto.commentingOpenDate) {
        this.logger.debug(
          `Cannot change commenting open date once state is ${entity.workflowStateCode}.`
        );
        return false;
      }
    }
    ...
    if (WorkflowStateEnum.COMMENT_OPEN == entity.workflowStateCode) {
      if (
        DateTimeUtil.getBcDate(dto.commentingClosedDate)
          .startOf('day')
          .isBefore(
            DateTimeUtil.getBcDate(entity.commentingOpenDate)
              .startOf('day')
              .add(30, 'day')
          )
      ) {
        this.logger.debug(
          `Not allowed to make commenting closed date shorter.`
        );
        return false;
      }
    }
    ...
    if (WorkflowStateEnum.COMMENT_OPEN == entity.workflowStateCode) {
      if (
        entity.operationStartYear !== dto.operationStartYear ||
        ...
        this.logger
          .debug(`Cannot change "Start of Operation", "End of Operation", "FSP ID", "District",
    ...
    if (WorkflowStateEnum.COMMENT_CLOSED == entity.workflowStateCode) {
      if (
        entity.commentingOpenDate !== dto.commentingOpenDate ||
        entity.commentingClosedDate !== dto.commentingClosedDate ||
        entity.districtId !== dto.districtId
      ) {
        this.logger.debug(
          `Cannot change commenting closed date for state ${entity.workflowStateCode}.`
        );
        return false;
      }
    }
  ...
  async isViewAuthorized(entity: Project, user?: User): Promise<boolean> {
    ...
    return (
      user.isForestClient && user.isAuthorizedForClientId(entity.forestClientId)
    );
  }
  ...
  async create(request: any, user: User): Promise<ProjectResponse> {
    ...
    this.validateProjectPlan(
      request.projectPlanCode,
      request.fspId,
      request.woodlotLicenseNumber
    );
    await this.validateTimberSalesManager(
      request.bctsMgrName,
      request.forestClientNumber,
      null
    );
  ...
  async validateTimberSalesManager(
    bctsMgrName: string,
    ...
    if (
      forestClientName &&
      forestClientName.toUpperCase().includes('TIMBER SALES MANAGER')
    ) {
      if (!bctsMgrName || bctsMgrName.length == 0) {
        throw new BadRequestException(
          'Timber Sales Manager name is required for Timber Sales FOM.'
        );
      }
    }
  }
  ...
  validateProjectPlan(
    ...
    if (projectPlanCode == ProjectPlanCodeEnum.FSP) {
      if (!isNil(woodlotLicenseNumber) || !isEmpty(woodlotLicenseNumber)) {
        throw new BadRequestException(
          'Contains invalid input for Forest Stewardship Plan FOM. Woodlot licence number should be empty.'
        );
      }
    } else if (projectPlanCode == ProjectPlanCodeEnum.WOODLOT) {
      if (!isNil(fspId)) {
        throw new BadRequestException(
          'Contains invalid input for Woodlot Licence Plan FOM. FSP ID should be empty.'
        );
      }
    }
  }
  ...
  async delete(projectId: number, user?: User): Promise<void> {
    const attachments = await this.attachmentService.findAllAttachments(
      projectId,
      user
    );
    ...
    for (const attachmentResponse of attachments) {
      await this.attachmentService.deleteAttachmentObject(
  ...
  convertEntity(entity: Project): ProjectResponse {
    ...
    if (entity.commentingClosedDate) {
      response.commentingClosedDate = dayjs(entity.commentingClosedDate).format(
        this.DATE_FORMAT
      );
      if (entity.bctsMgrName) {
        // Note: special rule for BCTS FOMs: validity period is 3 years from commenting close date.
        response.validityEndDate = dayjs(entity.commentingClosedDate)
          .add(3, 'year')
          .format(this.DATE_FORMAT);
      }
    }
    if (entity.commentingOpenDate) {
      response.commentingOpenDate = dayjs(entity.commentingOpenDate).format(
        this.DATE_FORMAT
      );
      if (!entity.bctsMgrName) {
        // For Non-BCTS FOMs: validity period is 3 years from commenting open date.
        response.validityEndDate = dayjs(entity.commentingOpenDate)
          .add(3, 'year')
          .format(this.DATE_FORMAT);
      }
    }
    ...
    if (entity.forestClient != null) {
      response.forestClient = this.forestClientService.convertEntity(
        entity.forestClient
      );
    }
    ...
    response.commentClassificationMandatory =
      entity.commentClassificationMandatory;
  ...
  async refreshCache(): Promise<any> {
    ...
    findCriteria.includeWorkflowStateCodes.push(
      WorkflowStateEnum.COMMENT_CLOSED
    );
  ...
  async findPublicSummaries(
    findCriteria: ProjectFindCriteria
    ...
    this.logger.info(
      'findPublicSummaries - Querying database with criteria %o',
      findCriteria.getCacheKey()
    );
    const query = this.repository
      .createQueryBuilder('p')
      .select([
  ...
  async workflowStateChange(
    projectId: number,
    request: ProjectWorkflowStateChangeRequest,
    user: User
  ): Promise<ProjectResponse> {
    this.logger.debug(
      `${this.constructor.name}.workflowStateChange projectId %o request %o`,
      projectId,
      request
    );
    ...
    const entity: Project = await this.findEntityWithCommonRelations(
      projectId,
      options
    );
    if (!entity) {
    ...
    if (entity.revisionCount != request.revisionCount) {
      this.logger.debug(
        'Entity revision count ' +
          entity.revisionCount +
          ' dto revision count = ' +
          request.revisionCount
      );
      throw new UnprocessableEntityException(
        'The record you are trying to save has been changed since you retrieved it for editing. Please refresh and try again.'
      );
    }

    if (request.workflowStateCode == entity.workflowStateCode) {
      throw new BadRequestException(
        'Requested state is equal to the current state.'
      );
    }
    ...
    switch (request.workflowStateCode) {
      case WorkflowStateEnum.INITIAL:
        throw new BadRequestException(
          'Changing state back to INITIAL not permitted.'
        );

      case WorkflowStateEnum.PUBLISHED:
        if (entity.workflowStateCode != WorkflowStateEnum.INITIAL) {
          throw new BadRequestException(
            'Can only publish if workflow state is Initial.'
          );
        }

        await this.validateWorkflowTransitionRules(
          entity,
          WorkflowStateEnum.PUBLISHED,
          user
        );
        await this.updatePublicNoticeIfRequired(
          entity,
          WorkflowStateEnum.PUBLISHED
        );
        break;
      ...
      case WorkflowStateEnum.COMMENT_OPEN:
        throw new BadRequestException(
          'Requesting state change to COMMENT_OPEN is not permitted.'
        );

      case WorkflowStateEnum.COMMENT_CLOSED:
        throw new BadRequestException(
          'Requesting state change to COMMENT_CLOSED is not permitted.'
        );

      case WorkflowStateEnum.FINALIZED:
        if (entity.workflowStateCode != WorkflowStateEnum.COMMENT_CLOSED) {
          throw new BadRequestException(
            'Can only finalize if workflow state is Commenting Closed.'
          );
        }
      ...
      case WorkflowStateEnum.EXPIRED:
        throw new BadRequestException(
          'Requesting state change to EXPIRED is not permitted.'
        );

      default:
        throw new InternalServerErrorException(
          'Unrecognized requested workflow state ' + request.workflowStateCode
        );
    }
    ...
    const updateCount = (await this.repository.update(projectId, updatePartial))
      .affected;
    if (updateCount != 1) {
    ...
    this.logger.debug(
      `${this.constructor.name}.update result entity %o`,
      updatedEntity
    );
    ...
    if (request.workflowStateCode == WorkflowStateEnum.FINALIZED) {
      try {
        this.logger.info(
          `FOM ${updatedEntity.id} is finalized. Sending notification email to district ${updatedEntity.district.name}`
        );
        ...
        throw new InternalServerErrorException(
          'Problem sending FOM finalized notification email.'
        );
      }
    }
  ...
  async commentClassificationMandatoryChange(
    projectId: number,
    request: ProjectCommentClassificationMandatoryChangeRequest,
    user: User
  ): Promise<ProjectResponse> {
    this.logger.debug(
      `${this.constructor.name}.CommentClassificationMandatoryChange projectId %o request %o`,
      projectId,
      request
    );
    ...
    if (
      ...
      WorkflowStateEnum.COMMENT_CLOSED != workflowStateCode
    ) {
      throw new BadRequestException(
        'Can only change Comment Classification Mandatory indicator in Commenting Open or Commenting Closed status.'
      );
    }
    ...
    if (isNil(request.commentClassificationMandatory)) {
      throw new BadRequestException(
        'Must provide a value for requested field to change.'
      );
    }
   ...
   */
  async validateWorkflowTransitionRules(
    entity: Project,
    stateTransition: WorkflowStateEnum,
    user: User
  ) {
    ...
    if (
      projectPlanCode == ProjectPlanCodeEnum.FSP &&
      (isNil(fspId) || isNaN(fspId))
    ) {
    ...
    if (
      projectPlanCode == ProjectPlanCodeEnum.WOODLOT &&
      (isNil(woodlotLicenseNumber) || isEmpty(woodlotLicenseNumber))
    ) {
      throw new BadRequestException(`Unable to transition FOM ${entity.id} to ${stateTransition}. 
    ...
    if (WorkflowStateEnum.PUBLISHED === stateTransition) {
      ...
      if (
        isNil(entity.commentingOpenDate) ||
        !dayjs(entity.commentingOpenDate, this.DATE_FORMAT).isValid()
      ) {
        throw new BadRequestException(`Unable to transition FOM ${entity.id} to ${stateTransition}.  
      ...
      const dayDiff = DateTimeUtil.diffNow(
        entity.commentingOpenDate,
        DateTimeUtil.TIMEZONE_VANCOUVER,
        'day'
      );
      if (dayDiff < 1) {
        throw new BadRequestException(`Unable to transition FOM ${entity.id} to ${stateTransition}.  
      ...
      if (
        isNil(entity.commentingClosedDate) ||
        !dayjs(entity.commentingClosedDate, this.DATE_FORMAT).isValid()
      ) {
        throw new BadRequestException(`Unable to transition FOM ${entity.id} to ${stateTransition}.  
      ...
      const openClosedDatesDiff = DateTimeUtil.diff(
        entity.commentingOpenDate,
        entity.commentingClosedDate,
        DateTimeUtil.TIMEZONE_VANCOUVER,
        'day'
      );
      ...
      const proposed = submissions.filter(
        (s) => s.submissionTypeCode == SubmissionTypeCodeEnum.PROPOSED
      );
      if (!proposed) {
        throw new BadRequestException(`Unable to transition FOM ${entity.id} to ${stateTransition}.  
        Proposed submission is required.`);
      ...
      if (!_.isEmpty(publicNotices)) {
        ...
        if (!_.isEmpty(postDate)) {
          ...
          if (
            postDate &&
            !DateTimeUtil.isPNPostdateOnOrBeforeCommentingOpenDate(
              postDate,
              commentingOpenDate
            )
          ) {
    ...
    if (WorkflowStateEnum.FINALIZED === stateTransition) {
      ...
      const final = submissions.filter(
        (s) => s.submissionTypeCode == SubmissionTypeCodeEnum.FINAL
      );
      ...
      // Public Notice attached
      const publicNotices =
        await this.attachmentService.findByProjectIdAndAttachmentTypes(
          entity.id,
      ...
      if (entity.commentClassificationMandatory) {
        // by default this field is mandatory, only ministry can chang it from admin page.
        const publicComments = await this.publicCommentService.findByProjectId(
          entity.id,
          user
        );
        if (publicComments && publicComments.length > 0) {
          const unClassifiedComments = publicComments.filter(
            (p) => p.response == null
          );
  ...
  async batchDateBasedWorkflowStateChange(): Promise<any> {
    this.logger.info(
      'Starting batch process for date-based workflow state changes...'
    );
    ...
    const currentPublishedFomIds = await this.findFomIds(
      WorkflowStateEnum.PUBLISHED,
      today.format(this.DATE_FORMAT),
      false
    );
    await this.updateProjectsState(
      WorkflowStateEnum.COMMENT_OPEN,
      currentPublishedFomIds
    );
    ...
    const currentCommentOpenFomIds = await this.findFomIds(
      WorkflowStateEnum.COMMENT_OPEN,
      today.format(this.DATE_FORMAT),
      true
    );
    ...
    const currentFinalizedFomIds = await this.findFomIds(
      WorkflowStateEnum.FINALIZED,
      past.format(this.DATE_FORMAT),
      false
    );
    await this.updateProjectsState(
      WorkflowStateEnum.EXPIRED,
      currentFinalizedFomIds
    );

    this.logger.info(
      'Completed batch process for date-based workflow state changes...'
    );
  }
  ...
  async findProjectMetrics(
    ...
    response.totalInteractionsCount = await this.getDataSource()
    ...
    response.totalCommentsCount = await this.getDataSource()
      .getRepository(PublicComment)
      .createQueryBuilder('comment')
      .select('comment.id')
      .where('comment.projectId = :projectId', { projectId: id })
      .getCount();

    response.respondedToCommentsCount = await this.getDataSource()
      .getRepository(PublicComment)
      .createQueryBuilder('comment')
      .select('comment.id')
      .where('comment.projectId = :projectId', { projectId: id })
  ...
  async commentingClosedDateChange(
    projectId: number,
    request: ProjectCommentingClosedDateChangeRequest,
    user: User
  ): Promise<boolean> {
    this.logger.debug(
      `${this.constructor.name}.commentingClosedDateChange projectId %o request %o`,
      projectId,
      request
    );
    ...
    if (
      isNil(
        request.commentingClosedDate ||
          !dayjs(request.commentingClosedDate, this.DATE_FORMAT).isValid()
      )
    ) {
    ...
    const workflowStateCode = entity.workflowStateCode;
    if (
      WorkflowStateEnum.INITIAL != workflowStateCode &&
      WorkflowStateEnum.COMMENT_OPEN != workflowStateCode &&
      WorkflowStateEnum.PUBLISHED != workflowStateCode
    ) {
      throw new BadRequestException(
        'Can only change Commenting Closed Date at Initial, Published, or Commenting Open status.'
      );
    }
   ...
   */
  private async findFomIds(
    workflowStateCode: WorkflowStateEnum,
    date: string,
    forCommentCloseDate: boolean
  ): Promise<number[]> {
    this.logger.debug(
      `Find FOM with workflowState ${workflowStateCode} and ${
        forCommentCloseDate ? 'commenting_closed_date' : 'commenting_open_date'
      } equal or before: ${date}`
    );
    const queryResults = await this.repository
      .createQueryBuilder()
      .select('project_id')
      .where('workflow_state_code = :workflowStateCode', {
   ...
   * @param projectIds
   */
  private async updateProjectsState(
    workflowStateCode: WorkflowStateEnum,
    projectIds: number[]
  ): Promise<void> {
    ...
    if (workflowStateCode === WorkflowStateEnum.COMMENT_OPEN) {
      updateFields['commentingClosedDate'] =
        () => `CASE WHEN commenting_closed_date IS NULL THEN commenting_open_date + integer '30' 
    ...
    this.logger.debug(
      `${updatedCounts} FOM(s) for ${projectIds} were updated to ${workflowStateCode}`
    );
  }

  private async updatePublicNoticeIfRequired(
    ...
    if (WorkflowStateEnum.PUBLISHED == stateTransition) {
      ...
      if (!_.isEmpty(publicNotices) && !publicNotices[0].postDate) {
        ...
        await this.repository
          ...
          .execute();
        this.logger
          .debug(`FOM (${projectEntity.id}) is updated for public notice (${pnId}) 
