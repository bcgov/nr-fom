...
1. **Strict Type Safety Enforced**
   The backend workspaces (`api`, `libs`) must compile with `"strict": true` and `"noImplicitAny": true`. We do not bypass these rules to hide errors. Frontend client workspaces (`admin`, `public`) continue to compile under their existing non-strict settings until their compiler dependencies are upgraded.
